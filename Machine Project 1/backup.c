...
void moveCursor(){
	...
	if(cursorY == 23 && nextLine == 0){
    	cursorLoc = (cursorY * 80) + (cursorX / 2);
	}
	
	else if(cursorY == 23 && nextLine == 1){
    	cursorLoc = ((cursorY + 1) * 80) + (cursorX / 2);
    	nextLine = 0;
    }
	
    else cursorLoc = (cursorY * 80) + (cursorX / 2);
...
void clearScreen(){
	...
	for(i = 0; i < marCount; i++){
		for(k = 0; k < strlen(marArray[i].text); k++)
			marArray[i].text[k] = ' ';
...
void printInt(int number){
	...
	while(sentinel != 0){
		converted[j] = (char)((int)'0' + (number / sentinel));
...
void saveString(struct marquee *marquee, char *string){
	int i;
...
void addMarquee(){
	...
	saveString(&marArray[marCount], movement);
	marCount++;
	printChar('\n');
}
...
void checkCommand(){
	...
	else if(strcmp("say ", message) == 0){
		printStr(command + 4);
		printChar('\n');
	}

	if(strcmp("marquee ", nextLine) == 0){	
		for(i = 0; i < commandIndex - 8; i++)
			movement[i] = command[i+8];
		movement[i] = '\0';
